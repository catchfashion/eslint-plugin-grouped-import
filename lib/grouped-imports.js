            ...
            Program: (node) => {
                ...
                if (node.type === "Program") {
                    ...
                    const unalphabetcialUngroupedNode = ungroupedNodes.find((next, index) => {
                        ...
                        return (ungroupedNodes[index - 1].source.value >
                            next.source.value);
                    });
                    const importGroupsHasReport = importGroups.some(({ imports, groupPriority, groupName: commentKey }, importGroupIndex) => {
                        ...
                        if (importGroupIndex !== 0 &&
                            ...
                            context.report({
                                ...
                                fix: (fixer) => {
                                    const prevImportGroupTextStart = prevImportGroup.imports[0].node.loc
                                        .start.line - 2;
                        ...
                        if (unsequentialItem) {
                            const groupImportTextRanges = [];
                            const allGroupImportTexts = lodash_1.default.flatMap(unsequentialItem
                                ? lodash_1.default.sortBy(imports, (g) => g.pathPriority)
                                : imports, (g) => {
                                const start = g.node.loc.start.line - 1;
                                const end = g.node.loc.end.line;
                                groupImportTextRanges.push([start, end - 1]);
                        ...
                        const unalphabetcialItem = imports.find((next, index) => {
                            ...
                            return (prev.node.source.value >
                                next.node.source.value);
                        });
                        if (unalphabetcialItem) {
                            const groupImportTextRanges = [];
                            const allGroupImportTexts = lodash_1.default.flatMap(lodash_1.default.sortBy(imports, (i) => i.pathPriority, (i) => i.node.source.value), (g) => {
                                const start = g.node.loc.start.line - 1;
                                const end = g.node.loc.end.line;
                                groupImportTextRanges.push([start, end - 1]);
                                return lines.slice(start, end);
